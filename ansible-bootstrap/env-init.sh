...
mkdir shared_volume  
docker pull jamesbe/ansible-node-ubuntu:v1  
docker pull ubuntu:14.04  
docker network create mynetwork  
cd shared_volume
docker run --rm williamyeh/ansible:ubuntu14.04-onbuild echo hallo  
docker run --rm ubuntu:14.04 echo hallo  
docker run -d --name ansible --rm --network mynetwork -v $(pwd):/shared_volume jamesbe/ansible-node-ubuntu:v1 /bin/bash -c 'while true; do sleep 60; echo keepalive; done'  
